...
import { DollarSign, RefreshCw, Pencil, X, Check } from "lucide-react";
...
import { apiGetInlandPricing, apiGetCrossBorderPricing, apiUpdateInlandPricing, apiUpdateCrossBorderPricing } from "@/lib/api";

type VehicleType = "Bike" | "Van" | "Truck";
type Country     = "Kenya" | "Uganda" | "Tanzania";

interface InlandRate {
  id: string; country: Country; vehicle_type: VehicleType; currency: string;
  base_fee: number; per_kg_rate: number;
  zone_multiplier_city: number; zone_multiplier_suburb: number; zone_multiplier_upcountry: number;
}

interface CrossBorderRate {
  id: string; from_country: Country; to_country: Country;
  vehicle_type: VehicleType; currency: string; base_fee: number; per_kg_rate: number;
}

const countryFlag: Record<Country, string> = { Kenya: "🇰🇪", Uganda: "🇺🇬", Tanzania: "🇹🇿" };
...
export default function PricingManager() {
  ...
  const load = async () => {
    setLoading(true);
    const [inland, cross] = await Promise.all([
      apiGetInlandPricing(),
      apiGetCrossBorderPricing(),
    ]);
    setInlandRates(inland || []);
    setCrossBorderRates(cross || []);
    setLoading(false);
  };

  useEffect(() => { load(); }, []);

  const startInlandEdit = (rate: InlandRate) => { setEditingInland(rate.id); setInlandEdits({ ...rate }); };
  const cancelInlandEdit = () => { setEditingInland(null); setInlandEdits({}); };

  const saveInlandEdit = async () => {
    if (!editingInland) return;
    setSaving(true);
    try {
      await apiUpdateInlandPricing(editingInland, {
      ...
      toast({ title: "Rate updated!", description: "Inland pricing saved." });
      cancelInlandEdit(); load();
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally { setSaving(false); }
  };

  const startCrossEdit = (rate: CrossBorderRate) => { setEditingCross(rate.id); setCrossEdits({ ...rate }); };
  const cancelCrossEdit = () => { setEditingCross(null); setCrossEdits({}); };

  const saveCrossEdit = async () => {
    if (!editingCross) return;
    setSaving(true);
    try {
      await apiUpdateCrossBorderPricing(editingCross, {
        base_fee:    crossEdits.base_fee,
        per_kg_rate: crossEdits.per_kg_rate,
      });
      toast({ title: "Rate updated!", description: "Cross-border pricing saved." });
      cancelCrossEdit(); load();
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally { setSaving(false); }
  };
        ...
        <>
          ...
          <Card className="shadow-card">
            <CardHeader>
              ...
              <CardDescription>Formula: (Base Fee + Weight × Per KG Rate) × Zone Multiplier</CardDescription>
            </CardHeader>
              ...
              <Table>
                ...
                <TableBody>
                  {inlandRates.map((rate, i) => {
                    ...
                    return (
                      <motion.tr key={rate.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                        ...
                        <TableCell>
                          <Badge className={`text-[10px] ${vehicleColors[rate.vehicle_type]}`}>{rate.vehicle_type}</Badge>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          {isEditing
                            ...
                            : <span className="text-sm font-mono">{Number(rate.base_fee).toLocaleString()}</span>}
                        </TableCell>
                        <TableCell className="text-right">
                          {isEditing
                            ...
                            : <span className="text-sm font-mono">{Number(rate.per_kg_rate).toLocaleString()}</span>}
                        </TableCell>
                        <TableCell className="text-right">
                          {isEditing
                        ...
                        <TableCell className="text-right">
                          {isEditing ? (
                            <div className="flex items-center justify-end gap-1">
                              <Button size="sm" variant="ghost" onClick={cancelInlandEdit} className="h-7 w-7 p-0"><X className="h-3.5 w-3.5 text-destructive" /></Button>
                              <Button size="sm" onClick={saveInlandEdit} disabled={saving} className="h-7 w-7 p-0 bg-success hover:bg-success/90"><Check className="h-3.5 w-3.5 text-white" /></Button>
                            </div>
                          ) : (
                            <Button size="sm" variant="ghost" onClick={() => startInlandEdit(rate)} className="h-7 w-7 p-0"><Pencil className="h-3.5 w-3.5 text-muted-foreground" /></Button>
                          )}
                        </TableCell>
                      </motion.tr>
                    );
          ...
          <Card className="shadow-card">
            <CardHeader>
              ...
              <CardDescription>Formula: Base Fee + (Weight × Per KG Rate)</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {crossBorderRates.map((rate, i) => {
                    ...
                    return (
                      <motion.tr key={rate.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                        className={`border-b transition-colors ${isEditing ? "bg-secondary/5" : "hover:bg-muted/50"}`}>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          {isEditing ? (
                            <div className="flex items-center justify-end gap-1">
                              <Button size="sm" variant="ghost" onClick={cancelCrossEdit} className="h-7 w-7 p-0"><X className="h-3.5 w-3.5 text-destructive" /></Button>
                              <Button size="sm" onClick={saveCrossEdit} disabled={saving} className="h-7 w-7 p-0 bg-success hover:bg-success/90"><Check className="h-3.5 w-3.5 text-white" /></Button>
                            </div>
                          ) : (
                            <Button size="sm" variant="ghost" onClick={() => startCrossEdit(rate)} className="h-7 w-7 p-0"><Pencil className="h-3.5 w-3.5 text-muted-foreground" /></Button>
                          )}
                        </TableCell>
                      </motion.tr>
                    );
